...
AI 把三件事变便宜了：写作、计算、检索。于是人类的旧优势（记忆、文笔、套公式）开始贬值。新优势是：在多个看似合理的答案里选择一个最优的；检查推理链是否闭合，有没有偷换概念、因果倒置；主动找反例、找识别假设、找数据生成过程的污染。更危险的是反向螺旋：越把思考外包给 AI，批判主体越萎缩，而独立、清醒地思考恰恰是 AI 时代人类最珍贵的技能。
...
|   可被 Agent 加速    |          仍主要靠人          |
| :--------------: | :---------------------: |
| 文献综述、数据清洗、回归、图表  |        提出可识别的问题         |
| 公式推导、代码实现、复现已有研究 | 选择识别策略（IV、DID、RDD、结构模型） |
|   预测、分类、非线性拟合    |      制度与历史等经验知识判断       |

机器学习回答的是预测，政策与机制回答的是因果。Agent 越强，**不会计量经济学理论和方法的人会越快暴露**，他们会得到漂亮的系数和完全错误的故事。会方法的人则把 Agent 当成超级 RA：一夜就能学完过去一学期的稳健性知识和完成对应工作。

---

### 5. Agent 能做出来的就一定能做好，但需要规范

我在2026年暑假用 Agent 完成了一篇实证论文，算是打通了 AI 辅助科研的全流程。对此我的态度非常乐观：能力（capability）和可靠性（reliability）是两回事，**能做出来，就具备了做好的前提**；要稳定做好，必须靠 workflow 和 hook，而不是靠下一次运气。

LLM 是概率系统：同一任务，有时 90 分，有时 40 分。提示词管不住长尾，要把它变成生产系统，必须在概率外面加确定性约束。`Your AI isn’t failing you. Your workflow is.`
- **Workflow**：把任务拆分成不同的阶段（计划 → 执行 → 验证 → 修复），禁止一口气做完，防止模型越跑越偏。
...
2. **物理手脚**：传感器 + 执行器 + 实时反馈与控制，这才是具身智能（embodied / physical AI）。

人形被过度关注，是因为人的环境是按人设计的（门把手、楼梯、工具）。但经济上更早规模化的，往往是非人形：智能仓储、无人机、无人驾驶。形态服从任务，不服从科幻；“脑”已经很强，用“脑”操控“手脚”才是瓶颈，也才是下一轮产业主战场。
...
<div align="center">
  <img src='/images/subagent.png' width="300" />
</div>
